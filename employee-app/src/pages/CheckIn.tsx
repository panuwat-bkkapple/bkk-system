...
//
// ดีไซน์ต้นทาง (02): **แถบเลื่อนยืนยัน** (`SlideConfirm`) และ **ภาพรั้วพิกัด**
// (`Geofence` — วาดเอง ไม่โหลดแผนที่ ดูเหตุผลในไฟล์นั้น) ทำแล้วทั้งคู่
//
// **สแกน QR หน้างานยังไม่ได้ทำ และเป็นการตัดสินใจ ไม่ใช่การลืม** — QR ที่ติดไว้
// เฉยๆ ถ่ายรูปส่งให้เพื่อนได้ มันจึงไม่ได้พิสูจน์ว่าใครอยู่ตรงนั้น การทำให้
// พิสูจน์ได้ต้องเป็นรหัสที่หมุนบนจอที่สาขา ซึ่งเป็นระบบใหม่ทั้งชุด (อุปกรณ์ที่
// สาขา + นาฬิกาที่ตรงกัน + เส้นทางสำรองตอนจอดับ) ไม่ใช่ปุ่มบนแอป
export default function CheckIn({ fix }: { fix: GeoFix }) {
        ...
        <div className="card">
          ...
          {nearest && (
            <Geofence
              ...
              bearingDeg={bearingTo(fix, nearest.s)}
              inRange={inRange}
            />
          )}
