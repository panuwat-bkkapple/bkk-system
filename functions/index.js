...
const PUBLIC_TRACK_FIELDS_MIRROR = [
  ...
  "receive_method", "branch_name", "store_branch", "branch_details",
...
// ตัวรวมเงินบนงาน (adjustments ที่ applied + คูปองทุกใบ) ย้ายไป ./job-money.js
// เพื่อให้ callable ที่ต้องคิดยอดโอนฝั่ง server (confirmPayoutTransfer) ใช้ตัว
// เดียวกับ trigger ในไฟล์นี้ — สำเนายังนับเป็นตัวเดิมของ MIRROR 4 ที่ ไม่ได้เพิ่ม
...
// =============================================================================
// ค่าจ้างไรเดอร์เข้า P&L — บันทึกเป็นค่าใช้จ่ายตอนไรเดอร์ถอนเงิน
// ก่อนหน้านี้ค่าจ้างไรเดอร์ไม่เคยปรากฏใน /financial-report เลย กำไรรายเดือน
// จึงสูงเกินจริงเท่ากับค่าจ้างทั้งเดือน (logic ทั้งหมดอยู่ rider-fee-expense.js
// ตัวแยกค่าจ้างออกจากเงินคืนค่าทดรองอยู่ rider-cost-split.js)
// =============================================================================
Object.assign(exports, require("./rider-fee-expense").registerRiderFeeExpense());

// =============================================================================
// เบิกค่าใช้จ่ายที่ไรเดอร์สำรองจ่าย (ค่าทางด่วน/ค่าจอดรถ) — ครึ่งที่แอดมินรีวิว
// ครึ่งที่ไรเดอร์ยื่นอยู่คนละ codebase (`bkk-rider-app/functions/src/index.ts`
// ข้าง riderRequestWithdraw) เพราะตัวรีวิวต้องใช้ lookupStaffByAuth +
// dispatchAdminPush ซึ่งอยู่ที่นี่ (logic ทั้งหมดอยู่ rider-expenses.js)
// =============================================================================
Object.assign(
  exports,
  require("./rider-expenses").registerRiderExpenses({
    dispatchAdminPush,
    pushToRider,
    staffIdsByRoles,
  })
...
// =============================================================================
// สายการรับสมัครงาน — ย้ายการตัดสินใจจ้างมาไว้ฝั่ง HR (ใบสมัครยังถูกเขียนโดย
// ฟอร์ม /careers ของเว็บลูกค้าที่โหนดเดิม ไม่ได้ย้ายข้อมูล)
...
// เงื่อนไขถูก freeze ลงทะเบียนตอนออก พิมพ์ซ้ำได้ฉบับเดิมเป๊ะ ไม่เก็บไฟล์ลง Storage
// =============================================================================
Object.assign(exports, require("./hr-documents-api").registerHrDocuments());

// =============================================================================
// แฟ้มเอกสารพนักงาน — สำเนาบัตร ทะเบียนบ้าน สมุดบัญชี สัญญาที่เซ็นกลับ ล.ย.01
// สปส.1-03 ฯลฯ **ไฟล์เดินทางผ่าน callable ทั้งขาขึ้นและขาลง ไม่มี URL ที่ไหน
// เลย** เพราะ URL ของ Storage ที่มี download token ข้ามกฎได้ตลอดไป ซึ่งรับไม่ได้
// กับสำเนาบัตรประชาชนของพนักงาน (เหตุผลเต็มอยู่หัวไฟล์ hr-files.js)
// =============================================================================
Object.assign(exports, require("./hr-files-api").registerHrFiles());
...
// =============================================================================
// ลงเวลาเข้า-ออกงาน + ตารางเวร — ฝั่ง server ของ "แอปพนักงาน"
// กติกา (รั้วพิกัด กะข้ามเที่ยงคืน สาย/ออกก่อน) อยู่ hr-attendance.js ล้วน
// การตัดสินทั้งหมดอยู่ที่นี่ ไม่ใช่ในแอป — แอปส่งมาแค่พิกัด
// =============================================================================
Object.assign(exports, require("./hr-attendance-api").registerHrAttendance());

// =============================================================================
// แอปพนักงาน — ใบลา คำขอเปลี่ยนกะ และกล่องอนุมัติของหัวหน้า
// กติกาการลาใช้ตัวเดียวกับฝ่ายบุคคล (hr-leave.js) ไม่เขียนสูตรชุดที่สอง
