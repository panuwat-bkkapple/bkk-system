...
//
// ภ.ง.ด.1 ต้องการชื่อกับนามสกุลแยกช่อง แต่ทะเบียนเก็บ `name` ช่องเดียวมาตั้งแต่
// ต้นและมีคนอ่านเยอะมาก (สัญญา สลิป 50 ทวิ ภ.ง.ด.1ก บรรทัดในรอบจ่าย) —
// **`name` จึงกลายเป็นผลลัพธ์ที่ประกอบจากสองช่องใหม่ ไม่ใช่ถูกแทนที่**
{
  const pub = (o) => sanitizeEmployeePublic({ employment_type: "monthly", hired_at: 1, ...o });

  const split = pub({ title: "นาย", first_name: "สมชาย", last_name: "ใจดี" });
  check("ประกอบ name จากชื่อ+นามสกุล", split.value.name === "สมชาย ใจดี");
  check("เก็บคำนำหน้าแยก", split.value.title === "นาย");
  // **คำนำหน้าต้องไม่อยู่ใน name** — เอกสารทุกใบพิมพ์ `name` ต่อจากคำที่ระบุ
  // ความสัมพันธ์อยู่แล้ว ("กับ สมชาย ใจดี") การยัดคำนำหน้าเข้าไปทำให้ข้อความ
  // เดิมทั้งหมดอ่านแปลกโดยไม่ได้เพิ่มความถูกต้อง
  check("คำนำหน้าไม่อยู่ใน name", !split.value.name.includes("นาย"));

  // ช่องที่ประกอบได้ต้องชนะค่าที่ส่งมาเอง — มีที่เดียวที่เขียนความจริงเรื่องชื่อ
  check("ชื่อที่ประกอบชนะ name ที่ส่งมาด้วย",
    pub({ first_name: "ก", last_name: "ข", name: "ค่าเก่าที่ไม่ควรชนะ" }).value.name === "ก ข");

  // สายกดจ้างจากใบสมัครส่งมาแค่ชื่อเต็มที่ผู้สมัครกรอก — บังคับให้แยกตรงนั้น
  // แปลว่าต้องเดาแทนผู้สมัคร ซึ่งเป็นการเดาที่ไปโผล่บนแบบยื่นภาษี
  const legacy = pub({ name: "สมชาย ใจดี" });
  check("ยังรับชื่อเต็มช่องเดียวได้", legacy.value.name === "สมชาย ใจดี");
  check("ไม่แอบแยกให้เอง", legacy.value.first_name === null);
  check("ชื่อเต็มช่องเดียวไม่ error", legacy.errors.length === 0);

  check("กรอกชื่อแต่ไม่กรอกนามสกุล = error",
    pub({ first_name: "สมชาย" }).errors.some((e) => e.includes("นามสกุล")));
  check("กรอกนามสกุลแต่ไม่กรอกชื่อ = error",
    pub({ last_name: "ใจดี" }).errors.some((e) => e.includes("ชื่อ")));
}

// ── การเดาแยกชื่อ — **ข้อเสนอ ไม่ใช่ข้อสรุป** ──────────────────────────────
//
// ใช้เติมค่าเริ่มต้นให้ฟอร์มเท่านั้น ห้ามเขียนลงฐานข้อมูลอัตโนมัติ:
// ชื่อ/นามสกุลไปโผล่บนแบบยื่นภาษี เดาผิดแล้วบันทึกเงียบๆ = ยื่นผิดโดยไม่มีใครรู้
{
  const s = (n) => splitThaiName(n);
  check("ชื่อ นามสกุล", s("สมชาย ใจดี").first_name === "สมชาย" && s("สมชาย ใจดี").last_name === "ใจดี");
  check("สองคำ = มั่นใจ", s("สมชาย ใจดี").confident === true);

  check("คำนำหน้าติดกับชื่อ", s("นายสมชาย ใจดี").title === "นาย");
  check("คำนำหน้าเว้นวรรค", s("นาย สมชาย ใจดี").title === "นาย");
  // **"นางสาว" ต้องถูกตัดก่อน "นาง"** ไม่งั้นเหลือ "สาว" ไปติดหัวชื่อ
  check("นางสาวไม่ถูกตัดเป็นนาง", s("นางสาวกชวรรณ วาสนโกมุท").title === "นางสาว");
  check("นางสาว: ชื่อไม่มีเศษคำนำหน้าติด", s("นางสาวกชวรรณ วาสนโกมุท").first_name === "กชวรรณ");

  // เกินสองคำ = ชื่อกลาง/นามสกุลหลายคำ เดาแล้วผิดได้จริง
  check("สามคำขึ้นไป = ไม่มั่นใจ", s("สมชาย ใจดี มากมาย").confident === false);
  check("สามคำขึ้นไป = ไม่เดาให้", s("สมชาย ใจดี มากมาย").first_name === null);
  check("คำเดียว = ไม่มั่นใจ", s("สมชาย").confident === false);
  check("คำเดียว = ไม่รู้ว่านามสกุลอะไร", s("สมชาย").last_name === null);
  check("ว่าง = ไม่มั่นใจ", s("").confident === false);
  check("ค่าที่ไม่ใช่สตริงไม่ทำให้พัง", s(null).confident === false && s(undefined).first_name === null);
}
