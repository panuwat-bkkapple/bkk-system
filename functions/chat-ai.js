...
const FAQ = [
  ...
  { c: "การรับซื้อ", q: "รับซื้อเครื่องผ่อนอยู่ได้ไหม", a: "ถ้าเครื่องยังผ่อนไม่หมดแต่ไม่ติด Blacklist (ไม่ล็อกเครือข่าย) รับซื้อได้ แต่ลูกค้าต้องรับผิดชอบค่างวดที่เหลือเอง และเครื่องต้องไม่มี Activation Lock/iCloud Lock" },
...
function buildSystemPrompt({ assistantName, pub, kb, customerBlock, inHours }) {
  ...
  return [
    ...
    `- ไม่รับซื้อเครื่องที่ติดล็อก iCloud/FMI (Find My), ติดล็อก MDM, หรือติด Blacklist/ล็อกเครือข่าย เด็ดขาด — ถ้าลูกค้าถามว่าติด iCloud/ติด MDM/ติด Blacklist รับไหม ตอบว่า "ไม่รับซื้อ" และแนะนำว่าถ้าปลดล็อกเรียบร้อยแล้วค่อยนำมาประเมินได้. ห้ามบอกว่ารับซื้อแล้วหักราคาเด็ดขาด`,
    `- เรื่องเครื่องที่ "ยังผ่อนไม่หมด" (ผ่อนกับไฟแนนซ์/บัตรเครดิต ไม่ใช่ล็อกเครือข่าย): อย่าตอบจากความจำ ให้เรียก get_faq ก่อนแล้วสรุปตามคำตอบในคลัง FAQ ของร้าน`,
