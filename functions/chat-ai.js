...
function buildSystemPrompt({ assistantName, pub, kb, customerBlock, inHours }) {
  ...
  return [
    ...
    `6.10.1 อาการที่ลูกค้าเล่าเป็นภาษาชาวบ้าน แล้ว "ตีความได้ 2 ทาง" (ห้ามฟันธงเอง — ทั้งฟันธงว่ารับ และฟันธงว่าไม่รับ): ตัวอย่างเคสจริง "แบต 86% แต่ใช้ไปสักพักเครื่องดับ เปิดมาอีกทีเหลือ 10%" — อาการนี้ตรงได้ทั้ง "แบตเตอรี่เสื่อม" (แค่หัก % ยังรับซื้อ) และ "เปิดไม่ติด/ค้าง/ดับเอง" (ร้านไม่รับซื้อ) ซึ่งผลต่างกันคนละโลก. วิธีทำ: (1) เรียก get_condition_questions ของรุ่นนั้นก่อน เพื่อดูว่าชุดประเมินจริงมีตัวเลือกอะไรบ้างและตัวไหนติดป้าย reject (2) ถาม "คำถามแยกทางเดียวสั้นๆ" ที่ชี้ขาดระหว่างสองตัวเลือกนั้น โดยตั้งคำถามจาก label ของ option จริง ไม่ใช่จากศัพท์เทคนิคของเราเอง (เช่น "เครื่องดับเองตอนแบตยังเหลือเยอะ หรือดับเพราะแบตหมดจริงๆ ครับ" พร้อม [ตัวเลือก: ดับเองทั้งที่แบตยังเหลือ | ดับเพราะแบตหมดเร็ว | ไม่แน่ใจ]) (3) ได้คำตอบแล้วค่อยเดินตามข้อ 6.10 ตามป้ายของ option ที่ลูกค้าเลือก (4) ลูกค้าตอบ "ไม่แน่ใจ" ให้ถือว่าเป็นอาการที่หนักกว่า (ปลอดภัยไว้ก่อน) และบอกตรงๆ ว่าต้องให้เจ้าหน้าที่ตรวจเครื่องก่อนจึงจะยืนยันได้. ห้ามเดาราคาให้อาการเด็ดขาด — ไม่มีตัวเลขไหนที่ "ประมาณให้" ได้เอง ทุกตัวเลขต้องมาจากการ์ดที่คิดจากตัวเลือกจริง. ถามแยกทางได้ "ครั้งเดียว" (ไม่นับรวมชุดถามสภาพตามข้อ 6.3) แล้วต้องเดินหน้าต่อ`,
...
// ---------------------------------------------------------------------------
const LOGIC_VERSION = "2026-08-01.1";
...
const LOGIC_BEHAVIORS = [
  ...
  { key: "condition", label: "ถามสภาพทีละเรื่อง", emoji: "🧾", rules: [
    ...
    "อาการที่ลูกค้าเล่าเองแล้วตีความได้ 2 ทาง (เช่น 'แบต 86% แต่ใช้ไปสักพักดับ' = แบตเสื่อม หรือ ดับเอง) ถามแยกทางสั้นๆ 1 ครั้งจากตัวเลือกจริง แล้วค่อยตัดสิน — ไม่แน่ใจ = ถือว่าหนักกว่าไว้ก่อน",
