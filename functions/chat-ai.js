...
// file, because pricing needs conversation input the overview does not have.
// See the header of service-facts.js.
const {
  OFFICIAL_FAQ_LINES,
  searchFaq,
  deliveryZonesFrom,
...
const CONDITION_TOPIC_WORDS = [
  ...
  // Functional faults are described in the customer's own words, not in the
  // option's wording: "ใช้ไปสักพักดับ" never contains the literal "ดับเอง".
  // Live gap: that sentence made the guard drop a REJECT answer, so a device
  // the shop does not buy would have been quoted at nearly full price. This
  // list is deliberately generous — for faults, missing a real symptom costs
  // far more than accepting one too readily.
  { test: /เปิดเครื่อง|ใช้งานทั่วไป|ทัช|กล้อง|ลำโพง|ไมโครโฟน|เชื่อมต่อ|ซิม|สัญญาณ/i, words: [
    "เปิดไม่ติด", "เปิดติด", "เปิดไม่", "ดับ", "ดับเอง", "ค้าง", "แฮง", "รีสตาร์", "รีเอง", "รีบูต",
    "วูบ", "เด้ง", "ดร็อป", "ดรอป", "ตัดเอง", "ปิดเอง",
    "ทัช", "กล้อง", "ลำโพง", "ไมค์", "ไมโครโฟน", "สัญญาณ", "wifi", "wi-fi", "ซิม", "โทรออก",
    "เสีย", "พัง", "มีปัญหา", "ไม่ทำงาน", "ใช้ไม่ได้", "ผิดปกติ", "ปกติ", "ใช้งานได้",
    "restart", "shut", "shuts", "reboot", "freeze", "hang", "die", "dies", "turns off", "turn off", "not working", "broken", "problem", "issue",
  ] },
];
...
const SERVICE_INFO_LINES = [
  ...
  `- การจ่ายเงิน (ห้ามอธิบายผิดขั้นตอน): ทุกช่องทาง = ตรวจสภาพเสร็จ ยืนยันราคา แล้ว "โอนเงินเข้าบัญชีหน้างานทันที" ตอนรับเครื่อง (ไม่มีเงินสด — ดูข้อช่องทางการจ่ายเงิน). Pickup = ไรเดอร์ตรวจ+โอนเงินให้ถึงหน้าบ้านเดี๋ยวนั้นเลย ไม่มีการเอาเครื่องกลับไปตรวจที่ร้านก่อนแล้วค่อยจ่ายทีหลัง. Store-in = โอนที่ร้าน. Mail-in = โอนทันทีหลังเครื่องถึงร้านและตรวจเสร็จ. ไม่ต้องรอหลายวัน`,
...
function buildSystemPrompt({ assistantName, pub, kb, customerBlock, inHours }) {
  ...
  return [
    ...
    `- เขียนเหมือนพนักงานแชทคนจริงพิมพ์เอง ไม่ใช่บทความ: ประโยคสั้นยาวสลับกันตามจังหวะ บางจังหวะตอบสั้นมากได้ ("ได้เลยครับ เดี๋ยวเช็คให้นะครับ") ใช้ภาษาพูดสุภาพแบบพนักงานไทย ("รบกวนขอ...หน่อยครับ" "เดี๋ยวผมดูให้ครับ") ห้ามใช้ bullet หรือเลขข้อกับลูกค้าเด็ดขาด และอย่าตอบเป็นเรียงความครบเป๊ะทุกข้อความ — ความสมบูรณ์แบบสม่ำเสมอเกินไปคือสิ่งที่ทำให้ดูเป็นบอทที่สุด`,
    `- ห้ามเอ่ยถึงความเป็น AI/บอท/ระบบอัตโนมัติของตัวเองโดยลูกค้าไม่ได้ถามเด็ดขาด (ห้ามพูด "ในฐานะ AI", "ผมเป็นระบบ...") — แนะนำตัวแค่ "มาติน ทีมงาน BKK APPLE". ถ้าลูกค้าถามตรงๆ ว่าเป็นบอท/AI ไหม: ตอบตามจริงสั้นๆ อย่างมั่นใจ ไม่แก้ตัวยืดยาว แล้วพากลับเข้าเรื่องทันที เช่น "ผมมาตินครับ เป็นผู้ช่วยอัตโนมัติของทีม ดูแลเรื่องประเมินราคาได้เต็มที่เลยครับ ถ้าอยากคุยกับเจ้าหน้าที่โดยตรง ผมส่งต่อให้ได้ทันทีครับ" — "ห้ามโกหกว่าเป็นคน" (ถูกจับได้ = เสียความเชื่อถือหนักกว่ามาก) และห้ามเถียงกับลูกค้าเรื่องนี้`,
    ``,
    `หลักการสูงสุด 3 ข้อ (สำคัญกว่ากฎอื่นทั้งหมด ถ้าขัดกันให้ยึด 3 ข้อนี้):`,
    `ก. ข้อมูล "ของร้าน" ต้องมาจากระบบเท่านั้น: ราคา, ค่าบริการ, โปรโมชั่น, สาขา, นโยบาย, เงื่อนไข, ขั้นตอนบริการของร้าน, สถานะออเดอร์ และสเปก/ตัวเลือกรุ่นที่ใช้คิดราคา — ต้องมาจาก tool หรือ "ข้อมูลที่ยืนยันแล้ว" ในโพรมป์นี้เท่านั้น ไม่มีแหล่ง = บอกว่าขอเจ้าหน้าที่ยืนยันแล้ว escalate ห้ามเดาเด็ดขาด (เหตุผล: คุณประเมินราคาเองได้แต่จะไม่ตรงกับราคาจริงในระบบที่ร้านจ่าย — ระบบคือความจริงเดียว)`,
    `ก2. นอกโซนข้อมูลของร้าน คุณคือผู้เชี่ยวชาญ Apple ตัวจริง — "ใช้ความรู้ทั่วไปตอบได้เต็มที่" เหมือนแอดมินเก่งๆ ที่ลูกค้าประทับใจ: วิธีใช้งานและ how-to ทุกชนิด (เช็คสุขภาพแบตเตอรี่, Sign out iCloud/ปิด Find My, สำรองข้อมูล/ย้ายเครื่อง, Factory Reset, unpair Apple Watch, วิธีดูรุ่น/ความจุ/ประเทศเครื่อง), อธิบายศัพท์เทคนิคเป็นภาษาคน, ความแตกต่างภาพรวมระหว่างไลน์สินค้า, คำแนะนำเตรียมเครื่องก่อนขาย — ตอบเลยทันที "ห้ามโยนเจ้าหน้าที่หรืออ้างว่าไม่มีข้อมูลในระบบ" สำหรับความรู้ทั่วไปแบบนี้ เพราะมันไม่ใช่ข้อมูลของร้าน. ข้อแม้ 2 ข้อ: (1) ห้ามให้ความรู้ทั่วไปลามไปเป็นตัวเลขราคา/ตัวเลือก variant ที่ใช้คิดเงิน — โซนนั้นกลับไปยึดข้อ ก เสมอ (2) เรื่องไหนไม่แน่ใจจริงๆ บอกตรงๆ ว่าไม่ชัวร์ แล้วแนะเท่าที่มั่นใจ`,
    ...
    `6.1.1 คำสั่งเจ้าของร้าน — "ทวนรุ่น" ก่อนเสมอ: ข้อความแรกที่ตอบหลัง search_models เจอรุ่นที่มีราคา ต้องขึ้นต้นด้วยทักทายสั้นๆ แล้ว "ทวนรุ่นที่เจอในระบบ" ให้ลูกค้าเห็นก่อนทุกครั้ง เช่น "สวัสดีครับ คุณลูกค้าต้องการขาย iPhone 13 ใช่ไหมครับ". กติกาของการทวน: (ก) ทวนด้วย "ชื่อรุ่นตามที่ระบบเจอ" ไม่ใช่ลอกคำที่ลูกค้าพิมพ์ — ลูกค้าพิมพ์ "ไอโฟน13" ต้องทวนว่า "iPhone 13" (จุดประสงค์คือให้ลูกค้าจับผิดได้ทันทีถ้าระบบเข้าใจรุ่นผิด ก่อนจะเดินไปไกลแล้วต้องรื้อ) (ข) ทวนแล้ว "เดินหน้าต่อในข้อความเดียวกันทันที" ตามขั้นที่ 3 ของข้อ 6 (ขอชื่อ/เบอร์ + คำถามสภาพเรื่องแรกพร้อมปุ่ม) ห้ามส่งข้อความสั้นๆ แค่ทวนรุ่นแล้วหยุดรอลูกค้ายืนยัน — เสียจังหวะและเพิ่มรอบสนทนาเปล่าๆ (ค) ห้ามพูดตัวเลขราคาในข้อความนี้ (กฎเก็บ contact ก่อนเผยราคายังอยู่) และ "ห้ามถามความจุ" ตรงนี้ ความจุถามตอนออกการ์ดตามข้อ 6 (ง) ทวนรุ่น "ครั้งเดียวตอนเจอรุ่น" เท่านั้น ห้ามขึ้นต้นทุกข้อความด้วยชื่อรุ่นซ้ำๆ (เคสจริงที่เคยพัง: ย้ำ "iPhone 13" ทุกบับเบิลจนดูเหมือนหุ่นยนต์) — ทวนใหม่ได้ต่อเมื่อลูกค้าเปลี่ยนไปคุยรุ่นอื่น (จ) ทักทายใส่เฉพาะข้อความแรกของห้อง ถ้าทักไปแล้วให้ทวนรุ่นอย่างเดียวไม่ต้องทักซ้ำ. ข้อยกเว้นที่ไม่ต้องทวน: รุ่นงดรับซื้อ (ปฏิเสธสุภาพทันทีตามข้อ 2.1), ชื่อกำกวมที่ต้องให้เลือกรุ่นก่อน (ถามเลือกรุ่นตามกฎเดิม), และรุ่นที่ไม่มีราคาในระบบ (โหมดรับ Offer ตามข้อ 6ข)`,
    ...
    `6.10.1 ลูกค้าเล่า "อาการ" มา (ไม่ใช่กดปุ่มเลือกสภาพ) = ให้ซักแบบช่างผู้เชี่ยวชาญ ห้ามฟันธงเองทั้งสองทาง (ทั้งฟันธงว่ารับ และฟันธงว่าไม่รับ). เหตุผล: อาการเดียวกันมาจากคนละสาเหตุ และผลต่างกันคนละโลก — เคสจริง "แบต 86% แต่ใช้ไปสักพักเครื่องดับ เปิดมาอีกทีเหลือ 10%" ตรงได้ทั้ง "แบตเตอรี่เสื่อม" (แค่หัก % ยังรับซื้อปกติ) และ "เปิดไม่ติด/ค้าง/ดับเอง" (ร้านไม่รับซื้อ). ขั้นตอน: (1) เรียก get_condition_questions ของรุ่นนั้นก่อน ดูว่าชุดประเมินจริงมีตัวเลือกอะไร ตัวไหนติดป้าย reject (2) ตั้งสมมติฐานสาเหตุที่ "พบบ่อยที่สุด" ก่อนเสมอ อย่าเดาสาเหตุที่แย่ที่สุด (3) ถามคำถามที่ "แยกสาเหตุได้จริง" เรียงลำดับแบบช่าง ทีละข้อ พร้อมปุ่ม — และคำถามพวกนี้ส่วนใหญ่ "อยู่ในชุดถามสภาพมาตรฐานอยู่แล้ว" (แบตกี่ % / เคยซ่อมหรือเปลี่ยนอะไหล่ไหม) ให้ยกมาถามก่อนเป็นข้อแรกๆ ไม่ใช่ถามเพิ่มเป็นรอบใหม่ (จึงไม่ขัดข้อ 6.3) (4) พูดสมมติฐานให้ลูกค้าฟังสั้นๆ เป็นภาษาคน ไม่ใช่ศัพท์ช่าง เพื่อให้ลูกค้ารู้ว่าเราถามไปทำไม ("อาการดับทั้งที่ยังเหลือ 86% ส่วนใหญ่มาจากแบตเก็บประจุไม่อยู่ครับ ขอเช็คสองเรื่องสั้นๆ") (5) ได้คำตอบครบแล้วค่อยเลือก option แล้วเดินตามข้อ 6.10 ตามป้ายของ option นั้น. คู่มืออาการ→สิ่งที่ต้องถาม (ถามจาก label จริงของชุดประเมิน ห้ามลอกคำถามนี้ไปทั้งดุ้นถ้าชุดนั้นไม่มี option รองรับ): ดับเองทั้งที่แบตยังเหลือ / % กระโดดลงฮวบ / ใช้ได้แป๊บเดียวหมด → สมมติฐาน "แบตเสื่อม เก็บประจุไม่อยู่" ถาม (ก) สุขภาพแบตกี่ % (ตั้งค่า > แบตเตอรี่ > สุขภาพแบตเตอรี่) (ข) เคยซ่อมหรือเปลี่ยนอะไหล่ไหม โดยเฉพาะเคยเปลี่ยนแบตมาหรือยัง (ค) เสียบสายชาร์จอยู่แล้วใช้งานได้ปกติไหม | เปิดไม่ติดเลย จอดำสนิท → ถามว่าชาร์จทิ้งไว้สักพักแล้วติดไหม เคยตกน้ำ/ตกพื้นไหม | ค้าง/รีสตาร์ตเองบ่อย → ถามว่าเป็นทุกแอปหรือแอปเดียว อัปเดต iOS ล่าสุดแล้วยังเป็นไหม เคยซ่อมไหม | ร้อนผิดปกติ / ชาร์จไม่เข้า → ถามว่าเปลี่ยนสาย/หัวชาร์จแล้วยังเป็นไหม เคยเปลี่ยนแบตไหม | จอลาย จอมีจุด ทัชไม่ติดบางจุด → ถามว่าเคยเปลี่ยนจอมาไหม เป็นตลอดหรือเป็นบางครั้ง. กติกาที่ห้ามพลาด: ห้ามเดาราคาให้อาการเด็ดขาด ทุกตัวเลขต้องมาจากการ์ดที่คิดจากตัวเลือกจริง | ห้ามให้คำแนะนำเชิงซ่อม/ประเมินค่าซ่อม เราซักเพื่อ "ตีราคาให้ถูกต้อง" ไม่ใช่เพื่อวินิจฉัยแทนศูนย์ | ลูกค้าตอบ "ไม่แน่ใจ/ไม่รู้" ทุกข้อ ให้ยึดทางที่หนักกว่าไว้ก่อน แล้วบอกตรงๆ ว่าต้องให้เจ้าหน้าที่ตรวจเครื่องก่อนจึงยืนยันราคาได้ ห้ามซักวน | ซักได้ไม่เกิน 2 คำถามแล้วต้องเดินหน้าต่อ ห้ามลากเป็นแบบสอบถาม`,
    ...
    `6.4.1 (บั๊กร้ายที่พลาดบ่อย — ห้ามหักเกิน): เลือก "ตัวเลือกสภาพที่แย่ที่สุดเท่าที่ลูกค้าพูดจริง" ห้ามยกระดับความเสียหายเองเด็ดขาด. ลูกค้าพูดว่า "ใช้งานได้ปกติทุกอย่าง" = จอ/ทัชสกรีน/กล้อง/ลำโพง/การเชื่อมต่อ = "ปกติ" (หัก 0) ห้ามไปใส่ "รอยขีดข่วนลึกมองเห็นชัด". ลูกค้าพูด "รอยตกที่มุมนิดหน่อย" = รอยเล็กน้อยที่มุมเท่านั้น ห้ามตีเป็นบุบหนัก. ลูกค้าไม่ได้บอก % แบต ห้ามใส่ "81%-85%" เอง (เว้นว่างให้ระบบถือว่าปกติ). ลูกค้าไม่ได้บอกเรื่องกล่อง/อุปกรณ์ ห้ามใส่ "มีเฉพาะตัวเครื่อง". ลูกค้าไม่ได้บอกเรื่องประกัน ห้าม "เดา" สถานะประกัน (มีประกัน/หมดประกัน) ใส่เองเด็ดขาด — แต่ถ้าลูกค้า "บอกเอง" (เช่น "ประกันหมดแล้ว", "ประกันเหลือ 6 เดือน") และชุดคำถามของรุ่นนั้นมีหัวข้อประกัน ให้ใส่ option ตามที่ลูกค้าบอกเข้า answers ด้วยเสมอ. การใส่สภาพแย่เกินจริงทำให้ราคาต่ำเกิน ลูกค้าไม่ขาย และเสียความเชื่อใจ`,
    `6.4.4 คำถาม "X มีผลต่อราคาไหม" (ประกันศูนย์ สี กล่อง อุปกรณ์ รอยแบบต่างๆ ฯลฯ): ห้ามตอบว่า "มีผล/ไม่มีผล" จากความจำเด็ดขาด — แต่ละรุ่นใช้ชุดคำถามไม่เหมือนกัน (บั๊กจริง: ลูกค้าถาม "ประกันหมดแล้ว ได้ราคาเดิมไหม" AI ตอบทันทีว่าประกันไม่มีผล ทั้งที่ชุดประเมินของ iPad Gen 11 มีหัวข้อประกันที่ต่างกันราวพันบาท). วิธีที่ถูก: ดูจากชุดคำถามจริงของรุ่น (แคตตาล็อกตัวเลือกในบล็อก "ใบเสนอราคาล่าสุด" หรือเรียก get_condition_questions) — ถ้ามีหัวข้อ/ตัวเลือกเรื่องนั้น = มีผล ให้ถามสถานะจริงจากลูกค้า (พร้อมปุ่มจาก option จริง) แล้วออกการ์ดใหม่ตามคำตอบ, ถ้าไม่มี = ตอบได้ว่ารุ่นนี้ไม่ใช้ปัจจัยนั้นในการประเมิน ราคาเดิม`,
    ...
    `12. คำถามพื้นที่บริการ/รับถึงที่/ค่าบริการรับเครื่อง: ห้ามตอบจากความจำ ต้องเรียก check_pickup_service เสมอ. ลูกค้าบอกทำเลเป็น "ชื่อสถานที่/ห้าง/จุดสังเกต" (เช่น เมเจอร์รัชโยธิน, เซ็นทรัลลาดพร้าว, เมืองทองธานี) ห้ามถามกลับว่า "อยู่ที่ไหน" เหมือนไม่รู้จัก — ส่งชื่อสถานที่นั้นตรงๆ (เติมเขต/จังหวัดที่คุณรู้ต่อท้ายได้ เช่น "เมเจอร์รัชโยธิน จตุจักร กรุงเทพ") เป็น area_text ระบบ geocode หาพิกัดเองได้ ถ้าระบบหาไม่เจอค่อยขอ เขต/อำเภอ + จังหวัด. และต้องส่ง model_id ของรุ่นที่คุยกันอยู่ไปด้วยทุกครั้ง (โปรฟรี/ส่วนลดค่าบริการผูกกับรุ่น — ไม่ส่ง = ลูกค้าไม่ได้โปรที่ควรได้). แจ้งเป็นค่าประมาณเสมอ ยอดจริงระบบคำนวณตอนลูกค้าปักหมุดที่หน้า Checkout. ถ้าผลมี free_pickup/promo_applied ให้บอกลูกค้าเป็นจุดขายทันที`,
    `12.1 ลูกค้าเปลี่ยน/เพิ่มทำเลใหม่ (เช่น "ถ้าเป็นที่ X ล่ะ") ต้องเรียก check_pickup_service ใหม่ทุกครั้ง — ห้ามคำนวณหรือเทียบเอาเองจากคำตอบทำเลก่อนหน้า`,
    `12.2 สาขา/หน้าร้านของเรามีเฉพาะตามข้อมูล get_branches เท่านั้น — ห้ามอ้างหรือใบ้ว่ามีหน้าร้านที่อื่นเด็ดขาด (บั๊กจริง: ลูกค้าถามค่าบริการรับที่ "เซ็นทรัลลาดพร้าว" AI ตอบว่าเป็น Store-in "นำเครื่องมาที่หน้าร้านเลย" ทั้งที่เราไม่มีสาขาที่นั่น — ชื่อเขตลาดพร้าวในที่อยู่สำนักงานใหญ่ไม่ใช่เซ็นทรัลลาดพร้าว). สถานที่ที่ลูกค้าเอ่ยคือ "จุดให้ไรเดอร์ไปรับ" เสมอ เว้นแต่ลูกค้าพูดเองว่าจะเดินทางมาที่สาขาของเรา และก่อนแนะนำ Store-in ต้องระบุชื่อ+ที่ตั้งสาขาจริงจากข้อมูลให้ลูกค้าเห็นชัด`,
...
const LOGIC_BEHAVIORS = [
  ...
  { key: "condition", label: "ถามสภาพทีละเรื่อง", emoji: "🧾", rules: [
    ...
    "รับทราบสั้นๆ หลากคำ แล้วถามต่อตรงๆ — ห้ามติดสูตร 'ดีครับ ... ขอถามต่อนะครับ' ซ้ำทุกข้อความ",
    "'X มีผลต่อราคาไหม' (ประกัน/กล่อง/สี) ตอบจากชุดประเมินจริงของรุ่นเท่านั้น — แต่ละรุ่นถามไม่เหมือนกัน ห้ามตอบจากความจำ",
    "ปุ่ม = คำตอบสำเร็จรูปเท่านั้น (ไม่มีปุ่มกับคำถามปลายเปิด เช่น ขอชื่อ/เบอร์)",
    "เรื่องที่ลูกค้าตอบแล้วห้ามถามซ้ำ",
    "ลูกค้าเล่า 'อาการ' มา = ซักแบบช่าง: ตั้งสมมติฐานสาเหตุที่พบบ่อยสุดก่อน (ดับเองทั้งที่แบตยังเหลือ → แบตเก็บประจุไม่อยู่) แล้วถามสิ่งที่แยกสาเหตุได้จริง — สุขภาพแบตกี่ % แล้วเคยซ่อม/เปลี่ยนอะไหล่ไหม — ไม่เกิน 2 คำถามแล้วเดินต่อ ไม่แน่ใจ = ยึดทางที่หนักกว่า",
...
// The reply flatly claims warranty status has no effect on the price. That is
// only true for models whose condition set has no warranty group — the guard
// below overrides the claim when the set DOES have one (live bug: iPad Gen 11
// has warranty options ~1,000 baht apart; the AI answered "ไม่มีผล" from
// memory without looking at the set).
function warrantyNoEffectClaim(text) {
  return /ประกัน[\s\S]{0,50}(?:ไม่มีผล|ไม่ได้มีผล|ไม่กระทบ|ไม่เกี่ยว)/.test(String(text || ""));
}
...
function waitPromiseIntent(text) {
  ...
  return (
    ...
    // "ผมขอเช็ครุ่น X นี้ในระบบก่อนนะครับ" (live: MacBook Neo — the model
    // skipped search_models and narrated a future check instead; the model
    // WAS in catalog with prices). "ในระบบ" keeps this away from legitimate
    // clarifying openers like "ขอเช็คให้ชัดก่อนนะครับ หมายถึงรุ่นไหนครับ".
    /ขอ(?:เช็ค|ตรวจสอบ)[\s\S]{0,40}ในระบบ[\s\S]{0,12}ก่อน/.test(t) ||
...
function callbackPromiseIntent(text) {
  ...
  // Causative / compound phrasings that dodge the "จะ" and "กลับ" anchors
  // (live: "เดี๋ยวผมให้เจ้าหน้าที่ตรวจสอบและแจ้งราคาให้เลยครับ" — MacBook
  // Neo). Factual past tense stays safe: "เจ้าหน้าที่ตรวจสอบแล้วพบว่า..."
  // has no แจ้ง within reach and no causative ให้ prefix.
  if (/(เจ้าหน้าที่|แอดมิน|ทีมงาน)[\s\S]{0,20}(เช็ค|ตรวจสอบ|ประเมิน|ยืนยัน)[\s\S]{0,30}แจ้ง/.test(r)) return true;
  if (/ให้(เจ้าหน้าที่|แอดมิน|ทีมงาน)(ช่วย)?(เช็ค|ตรวจสอบ|ประเมิน)/.test(r)) return true;
...
function makeToolExecutor({ db, convoId, convo, pub, dispatchAdminPush, tag, state, assistantName, customerText, lastCustomerText, conditionEvidence, assistantText, replyInEnglish, customerSentPhoto }) {
  return async function executeTool(name, input) {
    switch (name) {
      ...
      case "get_branches": {
        // Data from service-facts (shared with the /search overview); the
        // note below is the half that stays here, because it instructs a
        // CHAT — attach the map link, offer an escalation — and a search
        // results page has neither.
        const { central, branches: list } = await loadBranches(db);
      ...
      case "get_promotions": {
        const { coupons, pickup_fee_promotions: riderPromos } = await loadPromotions(db);
        return {
          coupons,
...
// The admin answer-web, framed FOR A CHAT. The rows come from service-facts
// (shared with the /search overview); this sentence does not, and must not —
// it orders the model to call escalate_to_human and to route quotes through
// create_quote_card, which a surface with no tools cannot do. Handing those
// instructions to the overview would produce a paragraph promising a handoff
// that never happens.
function buildKbGraphBlock(kbGraph) {
  const body = kbGraphRows(kbGraph);
  if (!body) return "";
  return [
    "",
...
function buildLastSearchBlock(lastSearch, now) {
  ...
  return [
    ...
    // History poisoning guard (real bug: the AI told a customer iPad Air 5
    // comes in 10.9"/12.9" — pure memory, 12.9" is a Pro size — and then
    // REPEATED it next turn to stay consistent with its own earlier message,
    // even after rule 2.2 shipped). Data beats chat history, explicitly.
    "สเปกจริง: รายการความจุ/ตัวเลือกข้างบนคือทั้งหมดที่รุ่นนั้นมี — ไม่มีแกนขนาดจอในรายการ = รุ่นนั้นมีขนาดเดียว ห้ามถาม 'จอกี่นิ้ว'. ถ้าข้อความเก่าของคุณเองในแชทเคยเสนอขนาด/ตัวเลือกที่ไม่อยู่ในรายการนี้ = ข้อความเก่านั้นผิด ให้ขอโทษสั้นๆ แล้วตอบตามข้อมูลนี้ทันที ห้ามพูดตามเดิมเพื่อความต่อเนื่อง",
  ].join("\n");
}
...
function registerChatAi({ dispatchAdminPush, dispatchOpsAlert }) {
  const chatWidgetAiReply = onValueCreated(
    ...
    async (event) => {
      ...
      let storeProfile = {};
      try {
        storeProfile = await loadStoreProfile(db);
      ...
      try {
        ...
        let kbGraphBlock = "";
        try {
          const kbGraph = await loadKbGraph(db);
          if (kbGraph) kbGraphBlock = buildKbGraphBlock(kbGraph);
        } catch (err) {
        ...
        // Warranty-claim guard — the draft says warranty doesn't affect the
        // price, but this model's condition set HAS a warranty group: replace
        // with the truthful ask, chips built from the set's real options.
        if (finalText && !state.escalated && warrantyNoEffectClaim(finalText) && Array.isArray(lastQuoteGroups)) {
          const wg = lastQuoteGroups.find((g) => /ประกัน/.test(String(g.title || g.name || "")));
          if (wg) {
            const wOpts = (Array.isArray(wg.options) ? wg.options : Object.values(wg.options || {}))
              .map((o) => String((o && (o.label || o.name)) || "").trim())
              .filter(Boolean)
              .slice(0, 6);
            if (wOpts.length >= 2) {
      ...
      const resp = await callClaudeResilient({
        ...
        system: [{
          ...
          text: [
            ...
            `- ภาษาของร่าง: ร่างทุกฉบับต้องเป็น "ภาษาเดียวกับข้อความล่าสุดของลูกค้า" — ลูกค้าคุยอังกฤษ = ร่างเป็นอังกฤษลื่นไหลระดับเจ้าของภาษา (โทนอบอุ่นมืออาชีพเหมือนกัน), ลูกค้าคุยไทย = ร่างไทย. ส่วน intent/situation/label ยังเขียนเป็นไทยเสมอ (แอดมินอ่าน ไม่ได้ส่งให้ลูกค้า)`,
            `- น้ำเสียงพนักงานคนจริง สุภาพ (ไทยลงท้ายครับ) ไม่ใช้ bullet/เลขข้อ/อีโมจิ ไม่เกิน 3-4 ประโยคต่อร่าง`,
...
module.exports = {
  ...
  suspendAssistant,
  __test: {
    // buildStoreProfileBlock and searchFaq now live in service-facts.js, and
    // buildKbGraphBlock is chat framing wrapped around rows from there — all
    // three re-exported so the existing harness keeps reaching them at the
    // same address.
    buildSystemPrompt,
    buildLastQuoteBlock,
