  ...
  const displayJobs = useMemo(() => {
    ...
    return list.filter(j => {
      ...
      if (workspace === 'B2C') {
        ...
        const isLogistics = ['Active Leads', 'Assigned', 'Accepted', 'Heading to Customer', 'Arrived', 'Shipping', 'Rider Returning', 'Package Received', 'In-Transit', 'Pending QC', 'Being Inspected', 'QC Review', 'Revised Offer', 'Negotiation', 'Price Accepted', 'Payout Processing', 'Waiting for Handover'].includes(j.status);
