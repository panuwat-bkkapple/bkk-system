  ...
  return (
    <tr className={`hover:bg-blue-50/30 transition-colors ${!item.isActive && 'bg-slate-50/50 opacity-60'}`}>
      <td className="p-4 pl-6">
        <StatusToggle isActive={item.isActive} onToggle={() => onToggleStatus(item)} />
      </td>
      <td className="p-4">
      ...
      <td className="p-4">
        <div className="flex items-center gap-3">
          <button onClick={() => onToggleFeatured(item)} className={`p-1.5 rounded-full ${item.isFeatured ? 'bg-amber-100 text-amber-500' : 'text-slate-300 hover:text-amber-300 transition'}`}>
