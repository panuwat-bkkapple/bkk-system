...
- **ค่าบริการรับเครื่อง = รายได้บริษัท (VAT):** บริษัทจด VAT — `pickup_fee` (ค่าบริการไรเดอร์ที่หักจาก payout) เป็น**รายได้ค่าบริการ** ถือเป็น VAT-inclusive. `serviceFeeBreakdown()` ใน `email.js` แตก base + VAT 7% ออกมา. voucher/admin Paid โชว์บรรทัด "รวมราคารับซื้อ − ค่าบริการ (รวม VAT) = ยอดรับสุทธิ" + รายละเอียด VAT (ทั้ง HTML + PDF). คนละก้อนกับค่าวิ่งที่จ่ายไรเดอร์ (expense + อาจมี WHT)
