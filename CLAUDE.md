...
- **Setup ครั้งเดียวก่อนใช้จริง:** สร้าง custom domain `app.getmobie.com` ชี้ site `getmobie-app` ใน Firebase console + เพิ่มโดเมนใน Auth Authorized domains + deploy rules จาก bkk-frontend-next + (optional) env `DEALER_PORTAL_BASE_URL` ใน functions/.env ถ้าไม่ใช่ค่า default
