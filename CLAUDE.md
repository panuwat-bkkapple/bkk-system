...
- **`order_reconciliation` = probe ตัวเดียวที่ตรวจ "ธุรกิจ" ไม่ใช่ "ของข้างนอก"** — เทียบคนที่ยิง `checkout_submit_attempt` กับคนที่จบที่ปลายทางใดปลายทางหนึ่ง (`order_completed` / `checkout_submit_blocked` / `checkout_submit_error`) ในหน้าต่างหน่วง 3 ชม.→30 นาที (คนที่เพิ่งกดอาจยังทำรายการอยู่). ไม่มีปลายทาง = ลูกค้ากดแล้วระบบเงียบ ซึ่ง server มองไม่เห็นเพราะไม่มี request มาถึง. **ห้ามใส่เบอร์/ชื่อลูกค้าลงข้อความแจ้งเตือน (PDPA)** — ชี้ไปหน้า Abandoned Carts ที่ gate สิทธิ์แล้วแทน. query ตาม index `timestamp` ไม่กวาดทั้ง node
