...
- **Paid = แยกข้อมูล sensitive:** ลูกค้าได้ "ใบเสร็จ" (สินค้า/ยอด/บัญชี mask) เท่านั้น; แอดมินกลางได้ `buildAdminPaidSummaryEmail` = สรุปเต็ม (order + payout + ผลตรวจ SickW GSX/FMI/iCloud จาก `job.sickw_check.last_check` + KYC จาก `/jobs_kyc/{jobId}` เลขบัตร mask 4 ตัวท้าย). **ห้ามใส่ SickW/FMI/KYC ในอีเมลลูกค้า** (PDPA). อ่าน snapshot ที่เก็บไว้ตอน inspection — **ไม่ call SickW API ซ้ำ**
