...
- **ใบกำกับภาษีค่าบริการ:** ที่ Paid ถ้ามี `pickup_fee` → ออก **ใบกำกับภาษี/ใบเสร็จรับเงิน** แยกสำหรับค่าบริการ (`buildTaxInvoicePdf` ตาม ม.86/4: เลขที่ running, ผู้ขาย=บริษัท สนญ., ผู้ซื้อ=ลูกค้า, มูลค่าบริการ + VAT แยก). เลขที่จองจาก atomic counter `/counters/tax_invoice_seq` (format `IV-000123`) เก็บที่ `jobs/{id}/tax_invoice` (กัน allocate ซ้ำ) + archive `tax_invoices/{jobId}.pdf`. แนบไปทั้งลูกค้า+แอดมินคู่กับใบสำคัญรับเงิน
