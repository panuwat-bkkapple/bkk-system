...
### สถานะ P5 (2 ก.ย. 2569 — ครึ่งแรกทำแล้ว)

**ตัดรอบวันที่ 20 จ่ายวันที่ 25** (เจ้าของงานตัดสิน) เก็บที่ `settings/hr` ไม่ฝังในโค้ด
งวดของเดือนกันยายนจึงเป็น 21 ส.ค. – 20 ก.ย. จ่าย 25 ก.ย.

ทำแล้ว: `functions/hr-payroll.js` (เครื่องคิดเงิน กติกาล้วน) +
`functions/hr-payroll-api.js` (6 callable) + หน้า `/payroll` + ส่งออก CSV
ภ.ง.ด.1 และประกันสังคม

**สามกฎที่ระบบถือไว้:**

1. **รอบหนึ่งงวดมีใบเดียว** — คีย์ด้วยงวด (`payroll_runs/2569-09`) ไม่ใช่ push id
   การกดซ้ำคือ "คำนวณใหม่" ไม่ใช่ "สร้างใบที่สอง" สองใบของงวดเดียวกันคือทางที่
   จ่ายเงินซ้ำ
2. **อนุมัติแล้วแก้ไม่ได้ และอัตราที่ใช้ถูกแช่ไว้กับรอบ** — ถ้ากฎหมายเปลี่ยน
   พรุ่งนี้ สลิปที่ส่งไปแล้วต้องยังอธิบายตัวเองได้ (รูปเดียวกับใบกำกับภาษี)
3. **รอบที่กรอกไม่ครบอนุมัติไม่ได้** — ลูกจ้างรายวันที่ยังไม่กรอกจำนวนวัน หรือ
   คนที่ยังไม่ได้ตั้งเงินเดือน จะทำให้ยอดผิดเงียบๆ

**ฟรีแลนซ์/ไรเดอร์ไม่เข้ารอบเงินเดือน** — เขาถูกจ่ายผ่านกระเป๋าเงินและหัก 3%
ตอนถอนอยู่แล้ว (`functions/rider-wht.js`) การใส่เข้ามาคือจ่ายซ้ำ มีเทสกันไว้ทั้ง
ที่เครื่องคิดเงินและที่ตัวกรองรายชื่อเข้ารอบ

**สิ่งที่ยังไม่ทำ (ครึ่งหลังของ P5):** สลิปเงินเดือน PDF · หนังสือรับรองหัก ณ
ที่จ่าย 50 ทวิ ของพนักงานประจำ · ภ.ง.ด.1ก รายปี · หน้าเตือนกำหนดนำส่งวันที่ 7
(ตอนนี้มีเฉพาะ `/wht-report` ของไรเดอร์)

**และสิ่งที่ตั้งใจไม่ทำ: ไฟล์ e-filing ของกรมสรรพากรและประกันสังคม** — รูปแบบ
ไฟล์ราชการมีสเปกของตัวเองที่ต้องมีเอกสารจริงถึงจะทำถูก การเดาแล้วส่งของที่ดู
เหมือนใช้ได้แต่ยื่นไม่ผ่าน แย่กว่าการบอกตรงๆ ว่ายังไม่มี ระบบจึงส่งออกเป็น CSV
ที่มีตัวเลขต่อคนครบสำหรับกรอกหรืออัปโหลดต่อ และเขียนไว้บนหน้าจอว่าไม่ใช่ไฟล์
e-filing

### เพดานประกันสังคม 2569 + หน้าตั้งค่า (3 ก.ย. 2569)

**ค่าตั้งต้นชุดแรกผิดสำหรับปี 2569 และเจ้าของงานเป็นคนจับได้** — เพดานค่าจ้าง
ประกันสังคมปรับจาก 15,000 เป็น **17,500** (สมทบสูงสุด 875 บาท/เดือน) พื้นยังเป็น
1,650 แต่สมทบต่ำสุดเก็บ **83 บาท** ซึ่งคือ 82.50 ปัดขึ้นเป็นจำนวนเต็มบาท จึงเพิ่ม
`round_to_baht` (ปิดได้ เผื่อกติกาการปัดไม่ตรงกับที่เข้าใจ — ต่างกันเฉพาะยอดที่
ลงท้าย .50)

**คู่ที่พลาดง่ายที่สุดและตอนนี้มีเทสตรึงแล้ว:** ขยับเพดานค่าจ้างแล้วลืมขยับ
`sso_allowance_cap` (เพดานลดหย่อนภาษี) — ถ้าเพดานลดหย่อนต่ำกว่า 12 × เงินสมทบ
สูงสุดต่อเดือน แปลว่า**เราคิดภาษีจากเงินที่ลูกจ้างไม่เคยได้รับ** สองค่านี้อยู่
คนละหัวข้อกันในหน้าตั้งค่า ความผิดพลาดจึงมองไม่เห็นด้วยตา หน้าตั้งค่าเตือนสด
ตอนพิมพ์ และเทสตรวจ invariant นี้กับค่าตั้งต้นทุกครั้ง

**หน้า `/hr-settings` (CEO/HR)** เป็นเจ้าของ `settings/hr` ทั้งก้อน: วันตัดรอบ/
วันจ่าย/ตัวหารสัดส่วน · อัตรา-พื้น-เพดาน-การปัดของประกันสังคม · ค่าใช้จ่ายเหมา
และค่าลดหย่อนทุกตัว · ขั้นบันไดภาษีแบบแก้ทั้งตาราง · คำนำหน้ารหัสพนักงาน

สองข้อห้ามของหน้านั้น ทั้งคู่มีเทส:

1. **ห้าม `set()` ที่ `settings/hr` ต้อง `update()` รายคีย์** — ใต้โหนดเดียวกันมี
   `employee_code_seq_by_year` เขียนทับทั้งก้อน = ตัวนับกลับไปเริ่มใหม่ แล้วรหัส
   พนักงานคนใหม่ซ้ำกับคนที่มีอยู่
2. **ไม่คำนวณเงินให้ดูในหน้าตั้งค่า** — เครื่องคิดเงินมีตัวเดียวฝั่ง server
   ตัวอย่างที่คำนวณเองตรงนั้นคือสูตรสำเนาที่สอง อยากเห็นผลให้กดคำนวณรอบใหม่
   ซึ่งกางที่มาอยู่แล้ว

ค่าที่หน้าตั้งค่าโชว์ว่า "ถ้าไม่ตั้งจะได้อะไร" ถูกตรึงให้ตรงกับ `DEFAULT_*` ของ
เครื่องคิดเงินด้วยเทส — ไม่ตรงเมื่อไหร่ หน้าตั้งค่าจะโกหกคนที่มาอ่าน

**และ injection จับที่เทสอีกสองครั้งในรอบนี้** (รวมเป็นสามครั้งในงาน HR ทั้งหมด):
ข้อ "ปิดการปัดเป็นบาทได้" เขียวทั้งที่ถอด `round_to_baht` ออกจาก
`resolvePayrollConfig` ไปแล้ว เพราะเทสประกอบ config object เองแทนที่จะเดินผ่าน
resolve จริง — **พิสูจน์แค่ว่าฟังก์ชันอ่านฟิลด์เป็น ไม่ได้พิสูจน์ว่าค่าที่แอดมิน
ตั้งเดินทางมาถึง** · และข้อ "หน้าตั้งค่าเตือน…" เขียวตอนเปลี่ยนชื่อตัวแปรเป็น
`capWarningDisabled` เพราะ regex ยัง match สตริงที่ยาวกว่า **เทสต้องเช็คว่าถูก
render ไม่ใช่ว่ามีชื่อตัวแปรอยู่ในไฟล์**

**อัตราทั้งหมดเป็นค่าตั้งต้นที่ต้องตรวจก่อนยื่นจริงรอบแรก ไม่ใช่ค่าที่ระบบ
รับประกันว่าถูกกฎหมายวันนี้** — สิ่งที่ทำให้ปลอดภัยคือ (ก) รอบถูกตรวจในสถานะ
draft ก่อนอนุมัติ โดยหน้าจอกางที่มาของทุกตัวเลข และ (ข) แก้ที่ `settings/hr` ได้
โดยไม่ต้อง deploy

**บทเรียนจากรอบนี้ — injection จับที่เทสอีกครั้ง ไม่ใช่ที่โค้ด:** เทสข้อ
"หน้าเว็บบอกว่า CSV ไม่ใช่ไฟล์ e-filing" **เขียวทั้งที่ลบข้อความนั้นออกจากหน้าจอ
ไปแล้ว** เพราะประโยคเดียวกันยังอยู่ในคอมเมนต์หัวไฟล์ซึ่งไม่มีลูกค้าคนไหนอ่าน —
**เทสที่ค้นทั้งไฟล์พิสูจน์แค่ว่าเคยมีคนเขียนคำนี้ไว้ ไม่ได้พิสูจน์ว่ามันยังขึ้น
บนจอ** แก้โดยตัดคอมเมนต์ออกก่อนค้นเสมอ และเทสตัวตัดคอมเมนต์เองด้วย
...
### สถานะ P1 (2 ก.ย. 2569 — ทำแล้ว)

ลงมือแล้วในรีโปนี้: `functions/hr-core.js` (กติกาล้วน) + `functions/hr.js`
(6 callable) + หน้า `/employees` + role `HR` + `src/utils/hrScope.ts`

**สามอย่างที่ P1 ตั้งใจ "ไม่ทำ" และเขียนไว้บนหน้าจอตรงๆ:**

1. **การตั้งสถานะพ้นสภาพไม่ได้ปิดบัญชี** — นั่นคือ P3 ระหว่างนี้ `accessSummary`
   คืนธง `stale_access` และหน้าเว็บขึ้นป้ายแดงว่าบัญชียังเข้าระบบได้ พร้อมบอก
   ว่าต้องไปปิดที่ไหน ระบบที่เขียนว่าพ้นสภาพแล้วเงียบเรื่องบัญชีที่ยังใช้ได้
   คือระบบที่ตอบผิดโดยไม่มีใครเห็น
2. **ไม่มี bulk import** — 4 คน กรอกมือ
3. **ไม่มี database rules ใหม่** — โหนดทั้งสามตกกฎ root `.read/.write:false`
   จึงอ่านผ่าน callable เท่านั้น ซึ่งถูกต้องแล้วสำหรับข้อมูลชุดนี้ rules ของ
   ข้อ 8 เป็นงานของ P2 ตอนที่พอร์ทัลต้องอ่าน realtime จริงๆ

**ขอบเขตของ role HR เป็นด่านระดับ UX ไม่ใช่ขอบเขตความปลอดภัย — ต้องพูดให้ตรง**
`hrScope.ts` กัน HR ไม่ให้เข้าหน้าปฏิบัติการ (route ในแอดมิน 28 เส้นทางไม่มี
guard เลย ค่าเริ่มต้นคือ "ล็อกอินแล้วเข้าได้") แต่ database rules ยังให้สิทธิ์
ตาม `/admins/{uid}` แบบเหมารวม คนที่มี role HR เปิด devtools อ่าน RTDB ตรงได้
เท่ากับ STAFF **การแยกสิทธิ์ระดับฐานข้อมูลตาม role คือเหตุผลที่พอร์ทัลต้องแยก
ตั้งแต่แรก ไม่ใช่ของที่ค่อยมาเติมทีหลัง**

**สิ่งที่ค้นพบตอนทำ ซึ่งเป็นเหตุผลของงานทั้งหมดนี้:** `sickw-sync-gate.test.mjs`
บังคับให้ `SICKW_SYNC_ROLES` เท่ากับ `VALID_ROLES` เป๊ะ — สมมติฐานที่ซ่อนอยู่คือ
"ทุก role ของพนักงานควรยิง SickW ได้" การเพิ่ม HR ทำให้เทสแดงทันที ซึ่งบังคับ
ให้ตอบคำถามว่าฝ่ายบุคคลควรใช้เงินบริษัทยิง SickW ไหม (ไม่) แทนที่จะเติมให้เทส
เขียว **เทสข้ามไฟล์ตัวนั้นทำงานตรงตามที่มันถูกเขียนมาเพื่อ**
